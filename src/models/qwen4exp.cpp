...
// for the batched PLE row prefetch below
#ifdef _WIN32
...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    for (int il = 0; il < (int) hparams.n_layer_all; ++il) {
        ...
        // Generic NextN exports also carry shared_head.norm. Qwen4Exp's head path uses the
        // HC mixer instead, so keep the tensor accepted but unused for now.
...
llama_model_qwen4exp::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // collect the wide residual to the output rows only when a masked nextn context is the
        // consumer (or in the normal non-nextn path); the unmasked MTP target keeps it full so
        // it can be exported as t_h_nextn below, mirroring qwen35moe
        if (il == n_layer - 1 && inp_out_ids && (cparams.embeddings_nextn_masked || !cparams.embeddings_nextn)) {
        ...
        // upstream applies the control vector via t_layer_inp, which this fork does not have,
        // so apply it explicitly on the wide HC residual (same as deepseek4)
        res_hc = build_cvec(res_hc, il);
        cb(res_hc, "l_last", il);
    }

    // export the wide pre-collapse residual for the MTP driver before the head mixer folds the
    // hc streams together; the target keeps it full-width (n_tokens rows) since it is read back
    // unmasked and fed straight into the draft head
    ...
    // in nextn mode the loop kept the wide residual (and hence cur) full-width, so collect the
    // per-token outputs down to the requested rows for the embedding/logit readback; the normal
    // path already collected res_hc in the loop, so cur is per-row here
    if (cparams.embeddings_nextn && inp_out_ids) {
        cur = ggml_get_rows(ctx0, cur, inp_out_ids);
    }
...
ggml_tensor * llama_model_qwen4exp::graph::build_qsa_top_k(
    ...
    // relu+permute copy - it scales with n_ubatch and at a wide ubatch is a multi-GiB slice of the reserved
    // compute buffer (smaller than glm5next's here since n_idx_h is 4, but still worth bounding). Score the
    // query tokens in chunks and top-k each chunk on its own: top-k is per query token, so the per-chunk
    // winners concatenated along the token axis are exactly the unchunked result while the peak transient is
    // bounded to one chunk. Shared knob with glm5next; LLAMA_DSA_LID_CHUNK_MB=0 restores the single-pass graph.
    ...
    // rope preserves shape; lay the roped query out as [idx_dim, n_idx_h, n_tps, n_stream] for token slicing
    ggml_tensor * q4   = ggml_reshape_4d(ctx0, ggml_cont(ctx0, q), idx_dim, n_idx_h, n_tps, n_stream);
    ...
    for (int64_t i0 = 0; i0 < n_tps; i0 += n_chunk) {
        ...
        ggml_tensor * q_cur = ggml_cont(ctx0, ggml_view_4d(ctx0, q4, idx_dim, n_idx_h, nc, n_stream,
                q4->nb[1], q4->nb[2], q4->nb[3], i0*q4->nb[2]));
        ...
        // build_attn_qsa reads [n_top_k, n_batch, 1, n_stream]; top-k per chunk, concat on the token axis
...
// Batch the PLE gather's page faults.
// per_layer_token_embd is ~27 GiB of IQ4_NL rows, and each token reads ple_n_heads of them at
// essentially random offsets with almost no reuse. The CPU get_rows that reads them is pinned to
// n_tasks = 1 (see the GGML_OP_GET_ROWS FIXME in ggml-cpu.c), so every cold mapped-file fault is
// paid serially on the critical path: measured ~98 us each, ~1.2 ms per token for 16 rows. One
// batched prefetch issues them all at once instead, which measured ~0.18 ms for the same 16 rows.
// A prefetch is a pure hint, so the gathered bytes - and the output - are unchanged.
//   LLAMA_PLE_PREFETCH=0  disable
//   LLAMA_PLE_PROBE=1     also touch every row afterwards and report what the faults cost
static void llama_ple_prefetch_rows(const ggml_tensor * tbl, const std::vector<int32_t> & idx, int64_t n_tokens) {
    static const bool do_prefetch = []() { const char * e = getenv("LLAMA_PLE_PREFETCH"); return !e || atoi(e) != 0; }();
    static const bool do_probe    = []() { const char * e = getenv("LLAMA_PLE_PROBE");    return  e && atoi(e) != 0; }();

    if (!do_prefetch && !do_probe) {
        return;
    }

    if (tbl == nullptr || tbl->data == nullptr || !ggml_backend_buffer_is_host(tbl->buffer)) {
        static bool warned = false;
        if (!warned) {
            warned = true;
            LLAMA_LOG_WARN("%s: skipped, the PLE table is %s\n", __func__,
                           tbl == nullptr || tbl->data == nullptr ? "missing" : "not host-readable");
        }
        return;
    }

    const size_t rowsz = ggml_row_size(tbl->type, tbl->ne[0]);
    char * const  base = (char *) tbl->data;

    int64_t t0 = ggml_time_us();

    if (do_prefetch) {
#ifdef _WIN32
        std::vector<WIN32_MEMORY_RANGE_ENTRY> ent(idx.size());
        for (size_t k = 0; k < idx.size(); ++k) {
            ent[k].VirtualAddress = base + (size_t) idx[k]*rowsz;
            ent[k].NumberOfBytes  = rowsz;
        }
        if (!PrefetchVirtualMemory(GetCurrentProcess(), (ULONG_PTR) ent.size(), ent.data(), 0)) {
            static bool warned = false;
            if (!warned) {
                warned = true;
                LLAMA_LOG_WARN("%s: PrefetchVirtualMemory failed, error %lu\n", __func__, (unsigned long) GetLastError());
            }
        }
#else
        for (size_t k = 0; k < idx.size(); ++k) {
            (void) madvise(base + (size_t) idx[k]*rowsz, rowsz, MADV_WILLNEED);
        }
#endif
    }
    ...
    const int64_t pf_us = ggml_time_us() - t0;
    t0 = ggml_time_us();

    // serial first/last-byte touch, so a row that straddles a page is paid for in full
    volatile char sink = 0;
    for (size_t k = 0; k < idx.size(); ++k) {
        const char * p = base + (size_t) idx[k]*rowsz;
        sink ^= p[0];
        sink ^= p[rowsz - 1];
    }
    (void) sink;

    // one accumulator per regime: a prefill call is interesting on its own, decode needs a window
    static int64_t n_call[2] = {0}, n_tok[2] = {0}, us_pf[2] = {0}, us_touch[2] = {0};
    const int r = n_tokens > 1 ? 0 : 1;

    n_call[r]   += 1;
    n_tok[r]    += n_tokens;
    us_pf[r]    += pf_us;
    us_touch[r] += ggml_time_us() - t0;

    if (r == 0 || n_call[r] >= 50) {
        LLAMA_LOG_WARN("ple probe [%s]: %" PRId64 " calls, %" PRId64 " tokens | prefetch %.2f us/token, touch %.2f us/token\n",
                       r == 0 ? "prefill" : "decode", n_call[r], n_tok[r],
                       (double) us_pf[r]/n_tok[r], (double) us_touch[r]/n_tok[r]);
        n_call[r] = n_tok[r] = us_pf[r] = us_touch[r] = 0;
    }
}
...
void llm_graph_input_ple::set_input(const llama_ubatch * ubatch) {
    ...
    llama_ple_prefetch_rows(pmodel.per_layer_tok_embd, idx, n_tokens);
