...
struct llama_model_loader {
    ...
    static const int TENSOR_READ_LAZY       = 1 << 4; // read rows on demand instead of loading whole tensor; requires mmap for now
    ...
    llama_files files;
    std::vector<std::string> file_paths; // path of each entry in `files` (empty for non-file handles)
    llama_ftype ftype;
    llama_fver  fver;
